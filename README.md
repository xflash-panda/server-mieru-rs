...
Rust implementation of the [mieru](https://github.com/enfein/mieru) proxy protocol server agent. Communicates with the panel via gRPC/QUIC (connect-rpc).
...
- **gRPC panel integration** — Connects to panel via QUIC/H3 (connect-rpc) for config, user sync, traffic reporting, and heartbeat
...
|----------|-----|---------|-------------|
| `--server_host` | `X_PANDA_MIERU_SERVER_HOST` | `127.0.0.1` | gRPC server host |
| `--port` | `X_PANDA_MIERU_PORT` | `8082` | gRPC server port |
| `--node` | `X_PANDA_MIERU_NODE` | *required* | Node ID |
| `--server_name` | `X_PANDA_MIERU_SERVER_NAME` | same as host | TLS SNI for panel connection |
| `--ca_file` | `X_PANDA_MIERU_CA_FILE` | system CA | Custom CA certificate path |
...
Release binary is at `target/release/server-mieru-agent`.
...
src/
  ...
  business/        — Panel type bridging (connect-rpc API manager)
